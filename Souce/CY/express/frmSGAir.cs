...
{
    public partial class frmSGAir : UserControl
    {
        string m_usersite = "";
        public frmSGAir()
        {
        ...
        private bool chk()
        {
            try
            {
                ...
                txtgnKG.Text = txtgnKG.Text.Trim();
                if (string.IsNullOrEmpty(txtgnKG.Text.Trim()))
                {
                    lblMsg.Text = "实重不能为空！";
                    txtgnKG.Focus();
                    return false;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                bool bResult = DA.Express.Insert_SgAir(lblCust.Text.Trim(), txtCust.Text.Trim(), cobExpress.SelectedValue.ToString(), cobExpress.Text, txtNo.Text.Trim(),
                    txtPcs.Text.Trim(), txtgnKG.Text.Trim(), lblgwkg.Text.Trim(), volwiehgt, txtL.Text.Trim(), txtW.Text.Trim(), txtH.Text.Trim(), m_usersite, pub.username);
                //string strMsg = company + ":" + txtNo.Text + "\r\n" + "重量：" + txtgnKG + "\r\n" + "已成功提交。";
                string strMsg = string.Format("快递号：{0}\r\n共{1}件\r\n重量{2}kg  泡重{3}kg\r\n提交已经成功！", this.txtNo.Text, txtPcs.Text, this.txtgnKG.Text, this.lblgwkg.Text);
